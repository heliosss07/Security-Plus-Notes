# CompTIA Security+ (SY0-701) Notes

This repository contains organized, professional study notes for the **CompTIA Security+ (SY0-701)** certification.  
Notes are aligned with **Professor Messer’s YouTube course** and the official **CompTIA exam objectives**, structured by domain and section number.

---

## Purpose

These notes are intended for **private study and review**.  
All content is paraphrased and summarized based on my own understanding of the concepts and screenshots taken when I am watching the videos.

---

## Domain Overview

1. [Domain 1: General Security Concepts](./Domain1-General-Security-Concepts/)
2. [Domain 2: Threats, Vulnerabilities, and Mitigations](./Domain2-Threats-Vulnerabilities-and-Mitigations/)
3. [Domain 3: Security Architecture](./Domain3-Security-Architecture/)
4. [Domain 4: Security Operations](./Domain4-Security-Operations/)
5. [Domain 5: Security Program Management and Oversight](./Domain5-Security-Program-Management-and-Oversight/)

**POGI MO**
